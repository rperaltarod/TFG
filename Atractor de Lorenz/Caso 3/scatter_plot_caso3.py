...
#fig.suptitle('Scatter plot hiperparámetros vs Loss (MSE) y MAE')
...
axes[1][0].set_xlabel("Nº de capas", fontsize=20)
...
axes[1][1].scatter(data["num_units"], data["Loss (MSE)"],
                   c=data["num_ensayo"], s=30, edgecolors="black")
axes[1][1].set_xticks([100,300,500])
axes[1][1].set_xlabel("Nº de neuronas", fontsize=20)
...
#axes[1][2].set_xticklabels(['zero','two','four','six'])
...
plt.savefig('scatter_plot_caso3_2.png', dpi=300)
plt.show()
