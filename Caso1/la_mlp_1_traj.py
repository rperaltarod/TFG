...
def main():
    ...
    if path.exists('la_mlp_1_traj.h5'):
        ...
        HP_L_RATE = hp.HParam('lrate', hp.Discrete([0.01, 0.001]))
        HP_BATCH_SIZE = hp.HParam('batch_size', hp.Discrete([40, 80]))
    ...
    x_error = [x[0] for x in error]
    y_error = [x[1] for x in error]
    z_error = [x[2] for x in error]
